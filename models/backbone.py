...
def eva_tiny(pretrained=None, **kwargs):
    ...
    if pretrained:
        ...
        for k in range(12):
            checkpoint['blocks.%d.mlp.w1.weight'%k] = checkpoint['blocks.%d.mlp.w12.weight'%k][:512]
            checkpoint['blocks.%d.mlp.w1.bias'%k] = checkpoint['blocks.%d.mlp.w12.bias'%k][:512]
            checkpoint['blocks.%d.mlp.w2.weight'%k] = checkpoint['blocks.%d.mlp.w12.weight'%k][512:]
            checkpoint['blocks.%d.mlp.w2.bias'%k] = checkpoint['blocks.%d.mlp.w12.bias'%k][512:]
            del checkpoint['blocks.%d.mlp.w12.weight'%k], checkpoint['blocks.%d.mlp.w12.bias'%k]
        model.load_state_dict(checkpoint, strict=False)
    return model, 192


def eva_tiny_mim(pretrained=None, **kwargs):
    model = EVAVisionTransformer(
        img_size=224, patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=2.6667, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_mean_pooling=False, xattn=True, intp_freq=True, pt_hw_seq_len=16, 
        naiveswiglu=True, rope=True, **kwargs)
    if pretrained:
        # checkpoint = torch.load('deit_base_distilled_patch16_384-d0272ac0.pth', map_location="cpu")
        ...
        pos_embed = nn.functional.interpolate(pos_embed, size=(14,14), mode='bicubic', align_corners=False)
        pos_embed = pos_embed.reshape(1, 192, 14*14).transpose(1,2)
        checkpoint['pos_embed'] = torch.cat([cls_pos_embed.unsqueeze(0), pos_embed], dim=1) #torch.Size([1, 197, 192])
        for k in range(12):
            checkpoint['blocks.%d.mlp.w1.weight'%k] = checkpoint['blocks.%d.mlp.w12.weight'%k][:512]
            checkpoint['blocks.%d.mlp.w1.bias'%k] = checkpoint['blocks.%d.mlp.w12.bias'%k][:512]
            checkpoint['blocks.%d.mlp.w2.weight'%k] = checkpoint['blocks.%d.mlp.w12.weight'%k][512:]
...
def eva_tiny_patch14_partial_finetune(pretrained=None, finetune_layers_num=0, **kwargs):
    model = EVAVisionTransformer(
        img_size=336, patch_size=14, embed_dim=192, depth=12, num_heads=3, mlp_ratio=2.6667, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_mean_pooling=False, xattn=True, intp_freq=True, pt_hw_seq_len=16, 
        naiveswiglu=True, rope=True, partial_finetune=True, **kwargs)
    if pretrained:
        checkpoint = torch.load(pretrained, map_location="cpu")["module"]
...
def eva_tiny_mim_partial_finetune(pretrained=None, finetune_layers_num=0, **kwargs):
    model = EVAVisionTransformer(
        img_size=224, patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=2.6667, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_mean_pooling=False, xattn=True, intp_freq=True, pt_hw_seq_len=16, 
        naiveswiglu=True, rope=True, partial_finetune=True, **kwargs)
...
def eva_tiny_mim_patch14_partial_finetune(pretrained=None, finetune_layers_num=0, **kwargs):
    model = EVAVisionTransformer(
        img_size=224, patch_size=14, embed_dim=192, depth=12, num_heads=3, mlp_ratio=2.6667, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_mean_pooling=False, xattn=True, intp_freq=True, pt_hw_seq_len=16, 
        naiveswiglu=True, rope=True, partial_finetune=True, **kwargs)
    if pretrained:
        checkpoint = torch.load(pretrained, map_location="cpu")["module"]
...
def eva_small(pretrained=None, **kwargs):
    model = EVAVisionTransformer(
        img_size=336, patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=2.6667, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_mean_pooling=False, xattn=True, intp_freq=True, pt_hw_seq_len=16, 
        naiveswiglu=True, rope=True, **kwargs)
    if pretrained:
        # checkpoint = torch.load('deit_base_distilled_patch16_384-d0272ac0.pth', map_location="cpu")
        ...
        checkpoint['patch_embed.proj.weight'] = nn.functional.interpolate(checkpoint['patch_embed.proj.weight'], size=(16,16), mode='bicubic', align_corners=False)
        #checkpoint['pos_embed'] = nn.functional.interpolate(checkpoint['pos_embed'], size=(16,16), mode='bicubic', align_corners=False)
        pos_embed = checkpoint['pos_embed'][:,1:,:].transpose(1,2).reshape(1, 384, 24, 24)
        # pos_embed = checkpoint['pos_embed'][:,1:,:].transpose(1,2).reshape(1, 192, 16, 16)
        cls_pos_embed = checkpoint['pos_embed'][:,0,:]
        # pos_embed = nn.functional.interpolate(pos_embed, size=(14,14), mode='bicubic', align_corners=False)
        pos_embed = nn.functional.interpolate(pos_embed, size=(21,21), mode='bicubic', align_corners=False)
        #pos_embed = pos_embed.reshape(1, 192, 14*14).transpose(1,2)
        pos_embed = pos_embed.reshape(1, 384, 21*21).transpose(1,2)
        checkpoint['pos_embed'] = torch.cat([cls_pos_embed.unsqueeze(0), pos_embed], dim=1) #torch.Size([1, 442, 192])
        for k in range(12):
...
def eva_small_partial_finetune(pretrained=None, finetune_layers_num=0, **kwargs):
    model = EVAVisionTransformer(
        img_size=336, patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=2.6667, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_mean_pooling=False, xattn=True, intp_freq=True, pt_hw_seq_len=16, 
        naiveswiglu=True, rope=True, partial_finetune=True, **kwargs)
    if pretrained:
        ...
        for k in range(12):
            ...
            checkpoint['blocks.%d.mlp.w1_patch.bias'%k] = checkpoint['blocks.%d.mlp.w12.bias'%k][:1024]
            checkpoint['blocks.%d.mlp.w1_det.bias'%k] = checkpoint['blocks.%d.mlp.w12.bias'%k][:1024]

            checkpoint['blocks.%d.mlp.w2_patch.weight'%k] = checkpoint['blocks.%d.mlp.w12.weight'%k][1024:]
            checkpoint['blocks.%d.mlp.w2_det.weight'%k] = checkpoint['blocks.%d.mlp.w12.weight'%k][1024:]

            checkpoint['blocks.%d.mlp.w2_patch.bias'%k] = checkpoint['blocks.%d.mlp.w12.bias'%k][1024:]
            checkpoint['blocks.%d.mlp.w2_det.bias'%k] = checkpoint['blocks.%d.mlp.w12.bias'%k][1024:]
...
def eva_small_patch14_partial_finetune(pretrained=None, finetune_layers_num=0, **kwargs):
    model = EVAVisionTransformer(
        img_size=336, patch_size=14, embed_dim=384, depth=12, num_heads=6, mlp_ratio=2.6667, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_mean_pooling=False, xattn=True, intp_freq=True, pt_hw_seq_len=16, 
        naiveswiglu=True, rope=True, partial_finetune=True, **kwargs)
    if pretrained:
        checkpoint = torch.load(pretrained, map_location="cpu")["module"]
...
def eva_small_mim_partial_finetune(pretrained=None, finetune_layers_num=0, add_attn_mask=False, **kwargs):
    model = EVAVisionTransformer(
        img_size=224, patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=2.6667, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_mean_pooling=False, xattn=True, intp_freq=True, pt_hw_seq_len=16, 
        naiveswiglu=True, rope=True, attn_mask=add_attn_mask, partial_finetune=True, **kwargs)
    if pretrained:
        # checkpoint = torch.load('deit_base_distilled_patch16_384-d0272ac0.pth', map_location="cpu")
        # checkpoint = torch.hub.load_state_dict_from_url(
        #     url="https://dl.fbaipublicfiles.com/deit/deit_base_distilled_patch16_384-d0272ac0.pth",
...
def eva_small_mim_partial_finetune_attn_sep(pretrained=None, finetune_layers_num=0, add_attn_mask=False, **kwargs):
    ...
    if pretrained:
        ...
        unfrozen_parameters = ['qkv_det', 'bias_det', 'proj_det']
        all_layers = list(range(12))
        if finetune_layers_num == 0:
            finetune_layers = []
        elif finetune_layers_num > 12:
...
def eva_small_mim_partial_finetune_attn_ffn(pretrained=None, finetune_layers_num=0, add_attn_mask=False, **kwargs):
    model = EVAVisionTransformer(
        img_size=224, patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=2.6667, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_mean_pooling=False, xattn=True, intp_freq=True, pt_hw_seq_len=16, 
        naiveswiglu=True, rope=True, attn_mask=add_attn_mask, partial_finetune=True, partial_finetune_attn=True, **kwargs)
    if pretrained:
        # checkpoint = torch.load('deit_base_distilled_patch16_384-d0272ac0.pth', map_location="cpu")
        # checkpoint = torch.hub.load_state_dict_from_url(
        #     url="https://dl.fbaipublicfiles.com/deit/deit_base_distilled_patch16_384-d0272ac0.pth",
...
def eva_small_mim_patch14_partial_finetune(pretrained=None, finetune_layers_num=0, **kwargs):
    model = EVAVisionTransformer(
        img_size=224, patch_size=14, embed_dim=384, depth=12, num_heads=6, mlp_ratio=2.6667, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_mean_pooling=False, xattn=True, intp_freq=True, pt_hw_seq_len=16, 
        naiveswiglu=True, rope=True, partial_finetune=True, **kwargs)
    if pretrained:
        checkpoint = torch.load(pretrained, map_location="cpu")["module"]
...
def eva_base(pretrained=None, **kwargs):
    model = EVAVisionTransformer(
        img_size=448, patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=2.6667, qkv_bias=True,
...
def eva_base_partial_finetune(pretrained=None, finetune_layers_num=0, **kwargs):
    model = EVAVisionTransformer(
        img_size=448, patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=2.6667, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_mean_pooling=False, xattn=True, intp_freq=True, naiveswiglu=True, 
        subln=True, rope=True, partial_finetune=True, **kwargs)
    if pretrained:
        checkpoint = torch.load(pretrained, map_location="cpu")["module"]
        for k in list(checkpoint.keys()):
            if 'rope' in k:
